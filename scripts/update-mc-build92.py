...
title = 'Citizens United Facts Master LocalBrain Architecture v1.0'

with open(root / 'data/localbrain-architecture.json', encoding='utf-8') as f:
    lb = json.load(f)
...
mc['localbrain_architecture'] = '/data/localbrain-architecture.json'
mc['localbrain_architecture_dashboard'] = '/mission-control/localbrain-architecture.html'

mc['executive'] = {
    'overall_completion': 92,
    'current_build': {'number': 92, 'title': title},
    'active_phase': 'LocalBrain Architecture → War Room',
    'last_completed': 'AI Institution',
    'next_build': {'number': 93, 'title': 'Executive war room & countdown dashboard components'},
    ...
    'localbrain_architecture_readiness': s['localbrain_architecture_readiness_pct'],
    ...
    'mc2_readiness': 42,
    'ai_engine_readiness': max(s['localbrain_architecture_readiness_pct'], mc.get('executive', {}).get('ai_engine_readiness', 34)),
...
mc['localbrain_architecture_inventory'] = {
    'readiness_score': s['localbrain_architecture_readiness_pct'],
    'localbrains_online': s['localbrains_online'],
    'localbrains_total': s['localbrains_total'],
    'executive_dashboard_live': s['executive_dashboard_live'],
    'inter_brain_communication': s['inter_brain_communication'],
}

for bar in mc['progress_bars']:
    if bar['id'] == 'overall':
        bar['value'] = 92
        bar['note'] = f"Build #92 — LocalBrain Architecture. {s['days_remaining']} days to Jan 2027."
    if bar.get('id') == 'ai_institution':
        bar['note'] = f"AI Institution · {s['localbrains_online']}/{s['localbrains_total']} LocalBrains online."

if not any(b.get('id') == 'localbrain_architecture' for b in mc['progress_bars']):
    mc['progress_bars'].append({
        'id': 'localbrain_architecture',
        'label': 'LocalBrain Architecture',
        'value': s['localbrain_architecture_readiness_pct'],
        'max': 100,
        'note': f"{s['localbrains_online']}/{s['localbrains_total']} brains · exec dashboard {'live' if s['executive_dashboard_live'] else 'planned'}.",
    })
else:
    for bar in mc['progress_bars']:
        if bar['id'] == 'localbrain_architecture':
            bar['value'] = s['localbrain_architecture_readiness_pct']
...
mc['builds'].insert(0, {
    ...
    'purpose': 'Federation of specialized LocalBrains — distributed institutional intelligence',
    'summary': (
        f"20 core LocalBrains, cross-brain communication, executive calendar, institutional "
        f"memory, executive dashboard. {s['localbrain_architecture_readiness_pct']}% readiness · "
        f"{s['localbrains_online']}/{s['localbrains_total']} online · "
        f"inter-brain {'live' if s['inter_brain_communication'] else 'not live'}."
    ),
    'files_created': [
        'data/localbrain-architecture.json',
        'docs/MASTER_LOCALBRAIN_ARCHITECTURE.md',
        'builds/092-localbrain-architecture.md',
        'mission-control/localbrain-architecture.html',
        'scripts/gen-localbrain-architecture.py',
        'scripts/update-mc-build92.py',
    ],
    'files_modified': [
        'js/mission-control.js', 'data/site.json', 'BUILD_PLAN.md', 'netlify.toml',
    ],
    'pages_created': ['/mission-control/localbrain-architecture.html'],
    'decisions_made': [
        'Federation of LocalBrains — no single AI/DB carries institution',
        '20 core LocalBrains with memory, calendar, tasks, knowledge, AI agents',
        'Mission Control = executive coordination layer above all brains',
        'Cross-brain automatic knowledge flow',
        'Modular expansion — future brains addable',
        f"{s['localbrain_architecture_readiness_pct']}% readiness — blueprint only",
    ],
    'open_questions': [
        'LocalBrain data store — per-brain DB or unified with partitions?',
        'Inter-brain messaging — event bus or MC webhooks?',
        'Executive calendar merge — real-time sync?',
        'When does a MC dashboard become a true LocalBrain node?',
    ],
    'risks': lb['catalog_gaps'][:4],
    'next_recommended': 93,
    'branch': 'main',
    'git_commit': 'pending',
    'netlify_deploy': 'https://arkansas-facts.netlify.app/mission-control/localbrain-architecture.html',
    'review_status': 'complete',
})

mc['briefing'] = {
    'what_built': 'LocalBrain Architecture v1.0 — Distributed Institutional Intelligence',
    'building_now': '92 builds — executive war room next',
    'blocked': ['LocalBrain nodes', 'Executive dashboard', 'Calendar merge', 'Inter-brain messaging'],
    'ready_public': ['20 brain registry', 'Cross-brain flow', 'Principle stack', 'Expansion model'],
    'next': 'Build #93 — Executive war room & countdown dashboard components',
}

if not any(n.get('id') == 'localbrain_architecture' for n in mc['build_map']):
    mc['build_map'].append({
        'id': 'localbrain_architecture',
        'label': 'LocalBrain Architecture',
        'href': '/mission-control/localbrain-architecture.html',
        'status': 'complete',
    })
